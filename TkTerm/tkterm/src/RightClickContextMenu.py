...
"""
        ...
        self.setting_win_top.geometry("800x600")
        self.setting_win_top.resizable(True, True)

        # hide the window
        self.setting_win_top.withdraw()

        # center the window
        self.setting_win_top.update_idletasks()
        min_width = 800
        min_height = 600
        x_coords = int(self.setting_win_top.winfo_screenwidth() / 2 - min_width / 2)
        y_coords = int(self.setting_win_top.wm_maxsize()[1] / 2 - min_height / 2)
        self.setting_win_top.geometry(f"{min_width}x{min_height}+{x_coords}+{y_coords}")
        self.setting_win_top.wm_minsize(min_width, min_height)

        self.setting_win_top.deiconify()
